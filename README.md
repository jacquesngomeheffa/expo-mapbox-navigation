...
> ⚠️ **Temporary iOS limitation (as of 3.1.9)**: `ios/ExpoMapboxNavigationModule.swift`/`ExpoMapboxNavigationView.swift` were reverted wholesale to youssefhenna/expo-mapbox-navigation's own proven files to unblock a real build failure — see the 3.1.9 changelog entry for the full reasoning. Only `coordinates`, `waypointIndices`, `mapStyle`, and `mute` are currently wired up on iOS. Every other prop below (`navigationProfile`, `language`, `excludeTypes`, `maxHeight`, `maxWidth`, `useMapMatching`, `customRasterTileUrl`, `customRasterAboveLayerId`, and all of "Color Customization" further down) is silently a no-op on iOS for now — not a crash, just no effect — while being fully implemented on Android as documented. These are being re-ported incrementally from `ios/_legacy_custom_swift_backup/`; this note will be removed prop-by-prop as each one is confirmed working again on iOS.
...
> ⚠️ **Temporary iOS limitation (as of 3.1.9)** — same cause as the Props note above: only `onRouteProgressChanged` currently fires with a matching name on both platforms. `onRoutesReady`, `onArrival`, `onNavigationCancelled`, `onNavigationFinished`, `onRoutesFailed`, and `onManeuverBannerPressed` below are Android-only for now — the current iOS source emits differently-named events (`onCancelNavigation`, `onWaypointArrival`, `onFinalDestinationArrival`, `onRouteChanged`, `onUserOffRoute`, `onRoutesLoaded`, `onRouteFailedToLoad`) that aren't yet wired up to this table's names. If your app's iOS flow depends on any event below firing, it silently won't until this is reconciled — check the 3.1.9 changelog entry.
