 ...
 *
 * Matching preforms asynchronously, so before to start matching you should
 ...
 *
 * @param cache            cache handle created with `CacheFactory`
 * @param config           RoadObjectMatcherConfig, created manually to adjust RoadObjectMatcher behavior
 */
- (nonnull instancetype)initWithCache:(nonnull MBNNCacheHandle *)cache
...
/**
 * WARNING: This API is not intended for public usage. It can be deleted or changed without any notice.
 * Sets the listener for obtaining matching results. Must be called before
 * any further match* calls.
 */
